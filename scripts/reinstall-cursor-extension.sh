...
EXTENSION_ID="mathrender.mathrender-vscode"
...
echo "Installing local Python mathrender package..."
"$ROOT_DIR/.venv/bin/python" -m pip install -e "$ROOT_DIR/python" numpy

echo "Building mathrender-lsp..."
cargo build -p mathrender_lsp
...
VSIX_PATH="$(find "$VSCODE_DIR" -maxdepth 1 -name 'mathrender-vscode-*.vsix' -type f -print | sort | tail -n 1)"
if [ -z "$VSIX_PATH" ]; then
