...
use mathdocs_metadata::{load_metadata_for_path, MetadataIndex};
...
impl RenderEngine {
    ...
    fn options_for_path(&self, path: Option<&Path>) -> RenderOptions {
        ...
        if let Some(config_path) = path.and_then(find_mathdocs_toml) {
...
fn find_mathdocs_toml(path: &Path) -> Option<PathBuf> {
    let mut dir = path.parent()?;
    loop {
        let candidate = dir.join("mathdocs.toml");
...
mod config_tests {
    ...
    #[test]
    fn finds_mathdocs_toml_in_parent_dirs() {
        let root = std::env::temp_dir().join(format!(
            "mathdocs-config-test-{}",
        ...
        std::fs::write(root.join("mathdocs.toml"), "auto_name = false\n").unwrap();
        let found = find_mathdocs_toml(&nested.join("example.py"));
        assert_eq!(found, Some(root.join("mathdocs.toml")));
