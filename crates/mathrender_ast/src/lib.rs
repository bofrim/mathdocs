...
pub fn parse_source(path: impl Into<String>, source: impl Into<String>) -> ParsedModule {
    ...
    if let Err(err) = ast::Suite::parse(&syntax_source, &source.path) {
        ...
        diagnostics.push(Diagnostic {
            code: "mathrender::syntax".to_string(),
...
fn sanitize_symbol_lookups_for_python_parse(source: &str) -> String {
    ...
    while let Some(start) = rest.find("::") {
        ...
        sanitized.push_str("__mathrender_symbol__");
...
pub fn scan_renderable_blocks(source: &SourceFile) -> Vec<RenderableBlock> {
    ...
    while line < source.line_count() {
        ...
        if let Some(command) = standalone_mathrender_command(trimmed) {
            match command {
                "off" => disabled = true,
                "on" => disabled = false,
                "ignore" => ignore_next = true,
                _ => apply_mathrender_config_command(&mut config, command),
            }
            line += 1;
            continue;
        }
        ...
        let (statement, block_config) = split_inline_mathrender_command(trimmed, config);
...
fn standalone_mathrender_command(trimmed: &str) -> Option<&str> {
    trimmed
        .strip_prefix("# mathrender:")
        .map(str::trim)
        .filter(|command| !command.is_empty())
}

fn split_inline_mathrender_command(
    trimmed: &str,
    ...
    let Some(command) = directive.strip_prefix("mathrender:").map(str::trim) else {
        return (trimmed, config);
    };
    apply_mathrender_config_command(&mut config, command);
    (trimmed[..comment].trim_end(), config)
}

fn apply_mathrender_config_command(config: &mut RenderConfig, command: &str) {
...
"#,
    ...
    #[test]
    fn accepts_symbol_lookup_syntax_for_mathrender_blocks() {
    ...
    #[test]
    fn applies_mathrender_config_commands_to_blocks() {
        let parsed = parse_source(
            "test.py",
            r#"
# mathrender: disable=auto_name/subscript, auto_name/symbol
final_loss = 1
alpha = 2 # mathrender: disable=auto_name/symbol
beta = 3
"#,
        );
        assert_eq!(parsed.blocks.len(), 3);
