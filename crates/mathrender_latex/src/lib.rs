use mathrender_ast::{Diagnostic, DiagnosticSeverity, TextRange};
use mathrender_ir::{qualified_name, BinaryOp, CompareOp, Expr, Statement, UnaryOp};
use mathrender_metadata::{MetadataIndex, SymbolMeta};
...
fn render_expr(
    ...
    match expr {
        ...
        Expr::Unsupported(text) => RenderedExpr {
            ...
            tensor_notation: false,
            diagnostics: vec![Diagnostic {
                code: "mathrender::unsupported".to_string(),
                ...
                (current.last().cloned(), operand_indices.first().cloned())
            {
                if left_last == right_first {
                    ...
                    diagnostics.push(Diagnostic {
                        code: "mathrender::tensor-index".to_string(),
                        message: format!(
            ...
            '{' => {
                ...
                if end < chars.len() && chars[end] == '}' && end > start {
                    ...
                    if let Some(arg) = args.get(arg_idx) {
                        ...
                        diagnostics.push(Diagnostic {
                            code: "mathrender::template-arity".to_string(),
                            message: format!(
...
mod tests {
    use super::*;
    use mathrender_ast::TextRange;
    use mathrender_ir::lower_statement;
    use mathrender_metadata::extract_inline_metadata;
...
"#;
    ...
    fn renders_symbol_lookup_expansions() {
        let metadata = mathrender_metadata::MetadataIndex::with_builtins();
