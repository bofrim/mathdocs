...
use mathrender_markdown::{RenderEngine, RenderedBlock, RenderedDocument};
use mathrender_metadata::symbol_lookup_entries;
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "MathRender language server initialized")
            .await;
    }
    ...
    async fn code_lens(&self, params: CodeLensParams) -> RpcResult<Option<Vec<CodeLens>>> {
        ...
        if let Some(range) = mathrender_import_range(&text) {
            lenses.push(CodeLens {
                range,
                command: Some(Command {
                    title: "Preview MathRender file".to_string(),
                    command: "mathrender.previewDocument".to_string(),
        ...
        lenses.extend(
            rendered
                ...
                .map(|block| CodeLens {
                    ...
                    command: Some(Command {
                        title: "Preview MathRender".to_string(),
                        command: "mathrender.previewRange".to_string(),
                        arguments: Some(vec![serde_json::json!({
                            "uri": uri,
                            "range": to_lsp_range(block.range),
                        })]),
    ...
    async fn completion(&self, params: CompletionParams) -> RpcResult<Option<CompletionResponse>> {
        ...
        let source = mathrender_ast::SourceFile::new(uri.to_string(), text);
...
impl Backend {
    ...
    async fn render_range(&self, params: RenderRangeParams) -> RpcResult<RenderedDocument> {
        ...
        let rendered = if let Some(range) = params.range {
            let source = mathrender_ast::SourceFile::new(display_path.clone(), text.clone());
    ...
    async fn publish_diagnostics(&self, uri: Url) {
        ...
        let display_path = uri.to_string();
        let source_file = mathrender_ast::SourceFile::new(display_path.clone(), text.clone());
        let rendered = self
            .engine
            .render_source(path.as_deref(), &display_path, &text);
...
fn render_diagnostic_to_lsp(
    ...
    source: &mathrender_ast::SourceFile,
) -> Diagnostic {
    Diagnostic {
        ...
        source: Some("mathrender".to_string()),
...
fn symbol_lookup_matches(entry: &mathrender_metadata::SymbolLookupEntry, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
fn mathrender_import_range(text: &str) -> Option<Range> {
    text.lines().enumerate().find_map(|(line_index, line)| {
        let trimmed = line.trim_start();
        if trimmed == "import mathrender"
            || trimmed.starts_with("import mathrender as ")
            || trimmed.starts_with("from mathrender import ")
        {
            Some(Range {
                start: Position {
